...
const NFTHeadBar = ({ themeToggling, setThemeToggling, menuToggling, setMenuToggling }) => {
      ...
      {({ isToggle, toggleTheme }) => (
        <AuthContext.Consumer>
          {({ isAuthStatus, setLogin, setLouout }) => (
            <header className='md:px-4 xs:px-1 sm:px-1 lg:px-8 py-6 flex items-center justify-between flex-wrap dark:bg-[#200A4C] text-gray-700 dark:text-gray-200 z-40'>

              <div className='lg:invisible flex items-center gap-4 xs:gap-1 justify-center z-50'>
                <img src={images.Logo} alt="" className='w-12 sm:w-10 xs:w-8 ' />
                <div>
                  <h1 className='lg:text-xl xl:text-xl 2xl:text-xl 3xl:text-xl sm:text-lg xs:text-xs md:text-2xl font-bold'>SLIDEGER</h1>
              ...
              <div className='flex items-center gap-4 ml-auto lg:mr-9 xl:mr-9 2xl:mr-9 3xl:mr-9 md:mr-8 sm:mr-4 xs:mr-2'>

                <div className='xs:hidden sm:hidden md:hidden lg:block xl:block 2xl:block 3xl:block bg-gray-300 rounded-2xl w-14 h-8 relative p-1 cursor-pointer dark:bg-[#4D398A]'
                  onClick={toggleTheme}>
